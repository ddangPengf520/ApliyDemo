                                  ...
                                  Seller:(NSString *)seller
{
    ...
    order.seller = seller;
    order.tradeNO = @"订单ID（由商家自行制定）";//订单ID（由商家自行制定）
...
- (NSString *)doRsa:(NSString *)orderInfo
{
    ...
    signer = CreateRSADataSigner(PartnerPrivateKey);// 商户私钥，自助生成
    NSString *signedString = [signer signString:orderInfo];
    
    //将签名成功字符串格式转化为订单字符串，格式固定
    NSString *orderString = nil;
    if (signedString != nil) {
        orderString = [NSString stringWithFormat:@"%@&sign=\"%@\"&sign_type=\"%@\"", orderInfo, orderString, @"RSA"];
        NSLog(@"orderString --- %@   orderInfo --- %@", orderString, orderInfo);
    }
    return orderString;
}
...
- (void)sendValueToAliPayWithOrderString:(NSString *)orderString FromScheme:(NSString *)appScheme
{
    [[AlipaySDK defaultService] payOrder:orderString fromScheme:appScheme callback:^(NSDictionary *resultDic) {
        ...
        //用SBJSON 或者JSONKit 将回调信息（字典）转化成字符串
...
- (void)paymentResult:(NSString *)resultDicToString
{
    ...
    if (result) {
        ...
        if (result.statusCode == 9000) {
             ...
             */
            // 交易成功
            NSString *key = AlipayPublicKey;//支付宝公钥
